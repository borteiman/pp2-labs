...
# this one is just to show food weight inside the cell
food_font = pygame.font.SysFont("Verdana", 18)
...
class Food:
    def __init__(self, snake):
        ...
        # here i store how many points this food gives
        self.weight = 1

        # timer starts when food appears
    ...
    def draw(self):
        # different weight = different color, easier to see in game
        if self.weight == 1:
            food_color = (0, 170, 0)
        elif self.weight == 2:
            food_color = (255, 165, 0)
        else:
            food_color = (180, 0, 180)

        pygame.draw.rect(screen, food_color, (self.pos.x * CELL, self.pos.y * CELL, CELL, CELL))

        # i put the weight number on the food so player sees its value
        weight_text = food_font.render(str(self.weight), True, colorWHITE)
        weight_rect = weight_text.get_rect(
            center=(self.pos.x * CELL + CELL // 2, self.pos.y * CELL + CELL // 2)
        )
    ...
    def generate_random_pos(self, snake):
        ...
        if free_cells:
            ...
            # random weight for every new food
            self.weight = random.randint(1, 3)

            # stronger food disappears faster so it is a bit more interesting
            if self.weight == 1:
                self.lifetime = 7000
            elif self.weight == 2:
                self.lifetime = 5000
            ...
            # save exact moment when this food spawned
            self.spawn_time = pygame.time.get_ticks()

    def is_expired(self):
        # if enough time passed, current food should vanish
...
# this one is separate, so level still depends on amount of foods, not weight
foods_eaten = 0
...
while running:
    ...
    if not game_over:
        ...
        if snake.check_food_collision(food):
            # now score grows by food weight, not always by 1
            score += food.weight

            # i keep normal food count separately for levels
            foods_eaten += 1

            food.generate_random_pos(snake)

            # level changes depending on how many foods were eaten
            ...
            # if player was too slow, old food disappears and new one spawns
            food.generate_random_pos(snake)
